...
def _get_otp_db_path():
    ...
    return os.path.join(data_dir, "otp.db")

def _init_otp_db():
    db_path = _get_otp_db_path()
...
def create_and_send_otp(phone):
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT created_at FROM otps WHERE phone = ?", (normalized,))
        row = cursor.fetchone()
        ...
        cursor.execute("""
            INSERT INTO otps (phone, otp, expires_at, attempts, created_at)
            VALUES (?, ?, ?, 0, ?)
        ...
        """, (normalized, otp, expires_at, now))
        conn.commit()
    
    return {
        "success": True,
        "message": "OTP logged to database (SMS provider not configured). Check DB.",
        "simulated": True,
    }
...
def verify_otp(phone, user_otp):
    ...
    with sqlite3.connect(db_path) as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT otp, expires_at, attempts FROM otps WHERE phone = ?", (normalized,))
        row = cursor.fetchone()
        ...
        if now > expires_at:
            cursor.execute("DELETE FROM otps WHERE phone = ?", (normalized,))
            conn.commit()
        ...
        attempts += 1
        if str(db_otp) != str(user_otp).strip():
            cursor.execute("UPDATE otps SET attempts = ? WHERE phone = ?", (attempts, normalized))
            conn.commit()
