...
def register_error_handlers(app):
    ...
    def unhandled_error(error):
        ...
        detail = traceback.format_exc() if os.getenv("FLASK_ENV") == "development" else None
        return error_response(
            f"Server error: {str(error)}" if os.getenv("FLASK_ENV") == "development" else "Internal server error", 
            500, 
            error_detail=detail
        )
