...
def update_subarea(subarea_id):
    ...
    try:
        ...
        # Ensure agent_ids exists and is a list (not None)
        subarea["agent_ids"] = subarea.get("agent_ids") or []
        ...
        if "agent_id" in payload:
            ...
            if agent_id is None:
                subarea["agent_ids"] = []
            elif agent_id not in (subarea.get("agent_ids") or []):
                subarea["agent_ids"] = (subarea.get("agent_ids") or []) + [agent_id]
            ...
            if agent_id:
                current_ids = subarea.get("agent_ids") or []
            ...
            if agent_id:
                subarea["agent_ids"] = [aid for aid in (subarea.get("agent_ids") or []) if aid != agent_id]
        ...
        update_agent_subareas_sync(subarea_id, subarea.get("agent_ids") or [])
...
def update_agent_subareas_sync(subarea_id, agent_ids):
    ...
    for agent in agents:
        agent_subareas = agent.get("sub_area_ids") or []
